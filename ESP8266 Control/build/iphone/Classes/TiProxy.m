...
//Rare exceptions to indicate a bug in the _esp8266control code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_esp8266control = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_esp8266control]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.esp8266control.module"
